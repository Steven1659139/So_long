...
void	set_image(t_map *map, void *mlx)
{
	...
	map->player.img = mlx_new_image(mlx, 1920, 1080);
	...
	map->player.adresse = mlx_get_data_addr(map->player.img, &map->player.bits_per_pixel, &map->player.line_length, &map->player.endian);
	...
	map->player.img = mlx_xpm_file_to_image(mlx, map->player.path, &map->player.width, &map->player.height);
